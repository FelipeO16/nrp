...
Citizen.CreateThread(function()
    vRP.prepare("NL/inserir_veh_ems",
                "INSERT INTO garagem_ems(modelo, nome, veh_tipo, placa, quantidade, img) VALUES(@modelo, @nome, @veh_tipo, @placa, @quantidade, @img)")
    vRP.prepare("NL/selecionar_veh_ems", "SELECT * FROM garagem_ems")
    vRP.prepare("NL/selecionar_by_model",
                "SELECT * FROM garagem_ems WHERE modelo = @modelo")
    vRP.prepare("NL/update_estoque_lspd",
                "UPDATE garagem_ems SET quantidade = @quantidade WHERE modelo = @modelo")
end)
----------------------------------------------------------------------------------------------------------------------------------------------------------
-- INIT GARAGEM
----------------------------------------------------------------------------------------------------------------------------------------------------------
...
function chamarEmergencia(user_id, source, msg)
    ...
    if user_id then
        ...
        if not in_ocorrencia then
                                    ...
                                    if not chamadaEMS[id .. '-' .. player] then
                                        ...
                                        if msg then
                                            local ok = vRP.request(player,
                                                                   "[Hospital-Central]: " ..
                                                                       identidade.name ..
                                                                       " está solicitando uma ambulancia, motivo: " ..
                                                                       htmlEntities.encode(
                                                                           msg),
                                                                   60)
                                            if ok then
                                                if not chamadaEMS[id .. '-' ..
                                                    player] then
                                                    if not atendido then
                                                        ...
                                                        vRPclient._notify(
                                                            source,
                                                            '[Hospital-Central]: Uma ambulancia está a caminho!')
                                                    else
                                                        vRPclient._notify(
                                                            player,
                                                            '[Hospital-Central]: Esta chamada já foi atendida!')
                                                    end
                                                else
                                                    vRPclient._notify(player,
                                                                      '[Hospital-Central] Você já está em um chamado!')
                                                end
                                            end
                                        else
                                            vRPclient._notify(source,
                                                              '[Hospital-Central]: Digite o motivo da chamada!')
                                        end
                                    end
                                end
                            end
            ...
            vRPclient._notify(source,
                              '[Hospital-Central]: Você já possui um chamado em aberto!')
        end
    end
end
...
AddEventHandler("vRP:playerLeave", function(user_id, source)
    local data = vRP.getUserDataTable(user_id)
    if data.emServico == "EMS" then vRP.updateServico(user_id, "Nenhum") end
end)
