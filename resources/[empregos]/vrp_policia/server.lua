...
Citizen.CreateThread(function()
    vRP.prepare("NL/inserir_veh_lspd",
                "INSERT INTO garagem_lspd(modelo, nome, veh_tipo, placa, quantidade, img) VALUES(@modelo, @nome, @veh_tipo, @placa, @quantidade, @img)")
    vRP.prepare("NL/selecionar_veh_lspd", "SELECT * FROM garagem_lspd")
    vRP.prepare("NL/selecionar_by_model",
                "SELECT * FROM garagem_lspd WHERE modelo = @modelo")
    vRP.prepare("NL/update_estoque_lspd",
                "UPDATE garagem_lspd SET quantidade = @quantidade WHERE modelo = @modelo")
    vRP.prepare("NL/delete_veh", "DELETE FROM garagem_lspd")
    vRP.prepare("NL/send_inspect",
                "INSERT INTO lspd(id, police_id, data, timer, `type`) VALUES (@id, @pid, @data, FROM_UNIXTIME(@timer), @type)")
    vRP.prepare("NL/delete_inspect",
                "DELETE FROM lspd WHERE id = @id AND timer = FROM_UNIXTIME(@timer)")
    vRP.prepare("NL/update_inspect",
                "UPDATE lspd SET `type` = '2' WHERE id = @id AND timer = FROM_UNIXTIME(@timer)")
end)
...
Citizen.CreateThread(function()
    ...
    vRP.prepare("NL/selecionar_arma_model",
                "SELECT * FROM arsenal_lspd WHERE modelo = @modelo")
    vRP.prepare("NL/inserir_arsenal",
                "INSERT INTO arsenal_lspd(modelo, nome, tipo, quantidade, img) VALUES(@modelo, @nome, @tipo, @quantidade, @img)")
    vRP.prepare("NL/update_estoque_arma_lspd",
                "UPDATE arsenal_lspd SET quantidade = @quantidade WHERE modelo = @modelo")
...
function emServico()
    ...
    local servico = vRP.table_size(vRP.table_filter(emServico, function(v, k, t)
        return v
    end))
    return servico
end
...
function chamarPolicia(user_id, source, msg)
    ...
    if user_id then
        ...
        if not in_ocorrencia then
                                    ...
                                    if not chamadaPolicia[id .. '-' .. player] then
                                        ...
                                        if msg then
                                            local ok = vRP.request(player,
                                                                   "[Policia-Central]: " ..
                                                                       identidade.name ..
                                                                       " está solicitando uma viatura, motivo: " ..
                                                                       htmlEntities.encode(
                                                                           msg),
                                                                   60)
                                            if ok then
                                                if not chamadaPolicia[id .. '-' ..
                                                    player] then
                                                    if not atendido then
                                                        ...
                                                        vRPclient._notify(
                                                            source,
                                                            '[Policia-Central]: Uma viatura está a caminho!')
                                                    else
                                                        vRPclient._notify(
                                                            player,
                                                            '[Policia-Central]: Esta chamada já foi atendida!')
                                                    end
                                                else
                                                    vRPclient._notify(player,
                                                                      '[Policia-Central] Você já está em uma ocorrência')
                                                end
                                            end
                                        else
                                            vRPclient._notify(source,
                                                              '[Policia-Central]: Digite o motivo da chamada!')
                                        end
                                    end
                                end
                            end
            ...
            vRPclient._notify(source,
                              '[Policia-Central]: Você já possui um chamado em aberto!')
        end
    end
end
...
function cancelarPolicia(user_id, source)
    ...
    if chamadaPolicia[player] then
        ...
        local playerSource = vRP.getUserSource(
                                 chamadaPolicia[player].id_policial)
        vRPclient.removeBlip(playerSource, chamadaPolicia[player].id_blip)
        vRPclient._notify(playerSource,
                          "[Policia-Central]: O cidadão cancelou o chamado!")
...
function pegarArmaDoJogador(player, arma)
    ...
    for k, v in pairs(armas) do if arma == k then return k end end
    return false
end
    ...
    if player then
        SetTimeout(10000, function()
            ...
            if parseInt(tempo) >= 1 then
                if tempo == 1 then
                    vRPclient._notify(player, "Você ainda vai passar <b>" ..
                                          parseInt(tempo) .. " mes</b> preso.")
                else
                    vRPclient._notify(player, "Você ainda vai passar <b>" ..
                                          parseInt(tempo) .. " meses</b> preso.")
                end
                vRP.setUData(target, "vRP:prisao",
...
function prisaoUniforme(target)
    ...
    if old_custom.modelhash == 1885233650 then
        ...
        SetTimeout(1000, function() vRPclient.setHealth(player, 400) end)
    else
        if not data.cloakroom_idle then
            idle_copy = vRP.save_idle_custom(player, old_custom)
        end
...
AddEventHandler('Policia:procurarVeiculo', function(placa)
    ...
    if #rows > 0 then
        for k, v in pairs(rows) do
            ...
            table.insert(infoVeh, {["Situação"] = "Normal"})
            TriggerClientEvent("Policia:infoVeh", source, dono, v.placa, v.img)
        end
    else
        vRPclient._notify(source,
                          "[Policia] Esse veiculo não está registrado!")
    end
end)
...
AddEventHandler('Policia:procurarPlayer', function(rg)
    ...
    if player then
        ...
        TriggerClientEvent("Policia:infoPlayer", source,
                           identidade.name .. ' ' .. identidade.firstname,
                           identidade.foto, identidade.age)
    else
    ...
    if #rows > 0 then
        ...
        if armaPlayer then
            ...
            if quantidade < 99 then
                ...
                vRP.execute("NL/update_estoque_arma_lspd",
                            {modelo = arma, quantidade = quantidade})
...
AddEventHandler('Policia:AtualizarEstoque', function(veh, tipo)
    ...
    if tipo then
        if #rows > 0 then
            ...
            quantidade = quantidade - 1 ]]
            vRP.execute("NL/update_estoque_lspd",
                        {modelo = veh, quantidade = 99})
        end
    else
        if #rows > 0 then
            local veiculo = rows[1]
...
RegisterCommand('rv', function(source, args, rawCommand)
    if not vRPclient.isHandcuffed(source) then
        ...
        if data.emServico == "LSPD" then
            local nplayer = vRPclient.getNearestPlayer(source, 3)
            if nplayer then vRPclient.ejectVehicle(nplayer) end
        end
    end
end)
        ...
        if data.emServico == "LSPD" then
            ...
            if id and tempo then
                ...
                if algemado then
                    ...
                    vRPclient._notify(source,
                                      "O meliante precisa estar algemado!")
                end
            else
                ...
                if nuser_id then
                    ...
                    if algemado and revista_algema then
                        ...
                        if #rList > 0 then
                            ...
                            TriggerClientEvent("vrp_policia:FoundItems", source,
                                               json.encode(rList), tm, nuser_id)
                        else
...
AddEventHandler('vrp_policia:apreenderRegistro', function(id, items, nuserid)
    ...
    for k, v in pairs(json.decode(items)) do
        if vRP.tryGetInventoryItem(parseInt(nuserid), v[1], v[3]) then
            if c4gather then
                vRP.giveInventoryItem(user_id, v[1], v[3])
            end
...
AddEventHandler("vRP:playerLeave", function(user_id, source)
    local data = vRP.getUserDataTable(user_id)
    if data.emServico == "LSPD" then vRP.updateServico(user_id, "Nenhum") end
end)
