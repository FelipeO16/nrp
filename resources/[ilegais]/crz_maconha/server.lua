...
AddEventHandler('crz_maconha:enviarWeed', function()
	...
	local player = vRP.getUserSource(user_id)
	if vRP.hasGroup(user_id,group) then
		if vRP.getInventoryWeight(user_id)+vRP.getItemWeight("folhamaconha")*6 <= vRP.getInventoryMaxWeight(user_id) then
			if vRP.giveInventoryItem(user_id, "folhamaconha", 6,true) then
				local typemessage = "sucesso"
				local mensagem = "Você recebeu <font color=\"green\">1</font> planta de maconha."
...
AddEventHandler('crz_maconha:enviarDroga', function()
	...
	local player = vRP.getUserSource(user_id)
	if vRP.hasGroup(user_id,group) then
        if vRP.tryGetInventoryItem(user_id, "folhamaconha", 12,true) then
