...
RegisterNetEvent('crz_maconha:entrada')
AddEventHandler('crz_maconha:entrada', function()
	DoScreenFadeOut(1000)
	Citizen.Wait(1500)
	SetEntityCoords(PlayerPedId(), 1066.26,-3183.41,-39.16)
	Citizen.Wait(1000)
	DoScreenFadeIn(1000)
end)
RegisterNetEvent('crz_maconha:saida')
AddEventHandler('crz_maconha:saida', function()
	DoScreenFadeOut(1000)
	Citizen.Wait(1500)
	SetEntityCoords(PlayerPedId(), -197.61,-1699.72,33.48)
	Citizen.Wait(1000)
	DoScreenFadeIn(1000)
end)
...
Citizen.CreateThread(function()
	while true do
		Wait(0)
		if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), -197.61,-1699.72,33.48, true) <= 1.0 then
			DrawText3Ds(-197.61,-1699.72,33.48+0.5,"PRESSIONE ~r~E~w~ PARA ENTRAR NO OFICIO")
			if IsControlJustPressed(0, 38) then		
				TriggerServerEvent('crz_maconha:entrada')
			end
		end	
		if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), 1066.26,-3183.41,-39.16, true) <= 1.0 then
			DrawText3Ds(1066.26,-3183.41,-39.16+0.5,"PRESSIONE ~r~E~w~ PARA SAIR DO OFICIO")
			if IsControlJustPressed(0, 38) then		
				TriggerEvent('crz_maconha:saida')
			end
        end	
    end
end)
...
local closest = {}
local cadeiras = {
	{objectId = -1281587804, x = 112.52, y = 6360.162, z = 32.78, h = 269.0}, -- cadeira DROGA   112.52102661133,6360.16796875,32.787044525146
}
...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), 112.52102661133,6360.16796875,32.787044525146, true) <= 1.5 or GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), 116.44131469727,6362.4877929688,32.785820007324, true) <= 0.8   then
            if closest then
