...
emp = Tunnel.getInterface("entrega_heroina")
...
-- INICIANDO TRABALHO
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent('entrega_heroina:permissao')
AddEventHandler('entrega_heroina:permissao',function()
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(1)
		if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), 283.65,-1924.37,26.06,true) <= 1 then
			DrawText3Ds(283.65,-1924.37,26.06+0.7,"PRESSIONE ~r~E~w~ PARA COMEÇAR MISSÃO")
            if IsControlJustPressed(0,38) then	
				TriggerEvent('entrega_heroina:permissao')
			end
		end
		if emservico then
			...
			if statuses then
				drawTxt(ui.right_x+0.0310,ui.bottom_y-0.046,1.0,1.0,0.35,"PRESSIONE ~r~U ~w~PARA CANCELAR A MISSÃO",255,255,255,150)
				drawTxt(ui.right_x+0.0310,ui.bottom_y-0.028,1.0,1.0,0.45,"ENTREGUE ~g~"..quantidade.."~w~ HEROINAS",255,255,255,255)
			else
				drawTxt(ui.right_x+0.0310,ui.bottom_y-0.028,1.0,1.0,0.35,"PRESSIONE ~r~Y ~w~PARA VER A MISSÃO",255,255,255,150)
			end
			if distance <= 50 then
				...
				if distance < 3 then
                    ...
                    if IsControlJustPressed(0,86) then
                        ...
                        RemoveBlip(blip)
                        TriggerServerEvent('entrega_heroina:itensReceber', quantidade)
                        while true do
