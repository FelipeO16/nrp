...
emp = {}
Tunnel.bindInterface("entrega_heroina",emp)
vRPclient = Tunnel.getInterface("vRP")
-----------------------------------------------------------------------------------------------------------------------------------------
-- FUNÇÕES
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent('entrega_heroina:permissao')
AddEventHandler('entrega_heroina:permissao',function()
	local source = source
	local user_id = vRP.getUserId(source)
	local player = vRP.getUserSource(user_id)
	if vRP.hasGroup(user_id,"CV") then
	    TriggerClientEvent('entrega_heroina:permissao', player)
	end
end)

RegisterServerEvent('entrega_heroina:itensReceber')
AddEventHandler('entrega_heroina:itensReceber', function(quantidade)
	local src = source
	local user_id = vRP.getUserId(src)
    local pagamento = math.random(700,1250)
    if user_id then
