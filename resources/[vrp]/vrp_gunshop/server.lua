...
vRP._prepare("NL/inserir_gunshops","INSERT INTO vrp_gunshop(id_dono, nome, preco, a_venda, estoque, cofre, cofreLimite) VALUES(@id_dono, @nome, @preco, @a_venda, @estoque, @cofre, @cofreLimite)")
vRP._prepare("NL/set_quantidade","UPDATE vrp_gunshop SET quantidade = @quantidade WHERE id = @id")
vRP._prepare("NL/get_estoque_gunshop","SELECT estoque FROM vrp_gunshop WHERE nome = @nome")
vRP._prepare("NL/set_estoque","UPDATE vrp_gunshop SET estoque = @estoque WHERE nome = @nome")
vRP._prepare("NL/get_cofre","SELECT cofre FROM vrp_gunshop WHERE nome = @nome")
vRP._prepare("NL/set_cofre","UPDATE vrp_gunshop SET cofre = @cofre WHERE nome = @nome")
vRP._prepare("NL/comprar_loja","UPDATE vrp_gunshop SET id_dono = @id_dono, a_venda = @a_venda WHERE nome = @nome")
vRP._prepare("NL/selecionar_loja_arma","SELECT * FROM vrp_gunshop")
vRP._prepare("NL/selecionar_loja_arma_nome","SELECT * FROM vrp_gunshop WHERE nome = @nome")
...
AddEventHandler('Gunshop:comprarArma', function(loja, arma)
    ...
    for k,v in pairs(weapon) do
        if estoque[v].quantidade > 0 then
            ...
            if ((vRP.getInventoryWeight(user_id)+vRP.getItemWeight("wbody|"..v)) <= vRP.getInventoryMaxWeight(user_id)) then
                ...
                if vRP.tryPayment(user_id, tonumber(preco)) then
                    ...
                    vRPclient._notify(source, "Você comprou uma <b>"..estoque[v].nome.."<b>")
                    ...
                    if id_dono ~= 0 then
                        if cofre ~= nil then
                            local dinheiro = { ["dinheiro"] = {amount = cofre["dinheiro"].amount + tonumber(preco)} }
                            setCofre(loja, dinheiro)
                        else
                            local dinheiro = { ["dinheiro"] = {amount = tonumber(preco)} }
                            setCofre(loja, dinheiro)
                        end
                    end
                else
...
AddEventHandler('Gunshop:comprarLoja', function(loja, preco)
    ...
    if vRP.tryPayment(user_id, tonumber(preco)) then
        ...
        enviarLojas()
        vRPclient._notify(source, "Parabéns! Você comprou a Ammunation: "..loja)
    else
        vRPclient._notify(source, "Você não tem dinheiro suficiente!")
    end
end)
...
AddEventHandler('NUI:ComprarWeapon', function()
