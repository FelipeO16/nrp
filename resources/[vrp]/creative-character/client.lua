...
RegisterNUICallback('cDoneSave',function(data,cb)
	...
	--HOPPE WELCOME CUTSCENE
	local ped = PlayerPedId()
    local x,y,z = table.unpack(GetEntityCoords(ped))
	
    --[[local cut = "mp_intro_concat"
    local cutscene = GetHashKey(cut)
	RequestCutsceneWithPlaybackList(cut,103,8)

	while not HasCutsceneLoaded(cut) do
        Wait(0)
	end
	RequestCollisionAtCoord(-1154.04296875,-1638.5858154297,3.9297955513)
	SetEntityCoords(ped, -1154.04296875,-1638.5858154297,3.9297955513)
    SetEntityVisible(ped, false)
	SetEntityInvincible(ped, true)

    --SetCutsceneOrigin(-1154.04296875,-1638.5858154297,3.9297955513)
	NetworkSetInMpCutscene(true, true)
	SetNetworkCutsceneEntities(true)
	SetLocalPlayerVisibleInCutscene(1, false)
	
    StartCutscene()
    Citizen.Wait(10000)
	local jet = GetHashKey("p_cs_mp_jet_01_s")
	RequestModel(jet)
	RequestModel(GetHashKey("mp_m_freemode_01"))
	
	
	while not HasModelLoaded(jet) do
		Citizen.Wait(50)
	end
	
	local f21 = CreateObject(jet, -1200.0, -1490.0, 142.38499450683594, 0, 0, 0)
	SetEntityLodDist(f21, 0xFFFF);
	--SetEntitySomething(f21, 0);
	SetEntityVisible(f21, 0, 0);
	StartParticleFxLoopedOnEntity("scr_mp_intro_plane_exhaust", f21, 5.403, -8.0, -2.2, 0.0, 0.0, 0.0, 0x3f800000, 0, 0, 0)
	StartParticleFxLoopedOnEntity("scr_mp_intro_plane_exhaust", f21, 6.629, -8.0, -2.2, 0.0, 0.0, 0.0, 0x3f800000, 0, 0, 0)
	
	RegisterEntityForCutscene(0, "MP_Male_Character", 3, GetHashKey("mp_m_freemode_01"), 0);
	RegisterEntityForCutscene(0, "MP_Female_Character", 3, GetHashKey("mp_m_freemode_01"), 0);
	...
	RegisterEntityForCutscene(f21,"MP_Plane", 0, 0, 0);
	RegisterEntityForCutscene(f21,"MP_Plane", 3, jet, 0);
	RegisterEntityForCutscene(PlayerPedId(),"MP_Plane", 0, 0, 0);
	RequestCollisionAtCoord(-1200.0, -1490.0, 142.38499450683594)
	--SetEntityCoords(ped, -1200.0, -1490.0, 142.38499450683594)
    Citizen.Wait(10000)
	RequestCollisionAtCoord(-1658.0703125,-2770.6447753906,16.102895736694)
	SetEntityCoords(ped, -1658.0703125,-2770.6447753906,16.102895736694)
	Citizen.Wait(10000)]]
	RequestCollisionAtCoord(-1038.6287841797,-2739.3715820313,20.169269561768)
	SetEntityCoords(ped, -1038.6287841797,-2739.3715820313,20.169269561768)
	TriggerEvent("ToogleBackCharacter")
