...
vRP._prepare("NL/inserir_venda","INSERT INTO nl_usados(dono, dono_nome, telefone, modelo, placa, preco, slot) VALUES(@dono, @dono_nome, @telefone, @modelo, @placa, @preco, @slot)")
vRP._prepare("NL/get_venda","SELECT * FROM nl_usados WHERE dono = @dono")
vRP._prepare("NL/get_vendas","SELECT * FROM nl_usados")

vRP._prepare("NL/get_venda_by_placa","SELECT * FROM nl_usados WHERE placa = @placa") -- Seleciona o veiculo apreendido pela placa

vRP._prepare("NL/remover_venda","DELETE FROM nl_usados WHERE slot = @slot")
vRP._prepare("NL/mover_veiculo","UPDATE vrp_user_vehicles SET user_id = @tuser_id WHERE user_id = @user_id AND vehicle = @vehicle")
vRP._prepare("NL/get_dinheiro","SELECT bank FROM vrp_user_moneys WHERE user_id = @user_id")
vRP._prepare("NL/set_banco","UPDATE vrp_user_moneys SET bank = @bank WHERE user_id = @user_id")
...
function vRPvd.colocarVenda(k)
    ...
    if #verificar_possui < cfg.limite_venda then
        ...
        if veh then
            local dono = vRP.query("vRP/get_veh_by_plate", {user_id = user_id, placa = placa})
            if #dono > 0 then
                local amount = vRP.prompt(source, "Preço que você quer colocar o veiculo a venda","")
                local valor = parseInt(amount)
                if valor > 0 then
                    local ok = vRP.request(source, "Você tem certeza que quer colocar seu carro a venda por <b>$"..valor.."</b> ?", 60)
                    if ok then
        ...
        if tuser_id ~= id_dono then
            ...
            if not possui then
                if vRP.tryPayment(tuser_id, preco_db) then
                    ...
                    vRP.execute('NL/mover_veiculo', {user_id = id_dono, tuser_id = tuser_id, vehicle = modelo_db})
                    vRP.execute('NL/remover_venda', {slot = k})
                    if online then
                        local banco = vRP.getBankMoney(id_dono)
                        vRP.setBankMoney(id_dono, preco_db+tonumber(banco))
                        vRPclient._notify(online, "[Loja de Usados] Seu veiculo foi vendido!")
                    else
                        local bank =  vRP.scalar('NL/get_dinheiro', {user_id = id_dono})
                        vRP.execute('NL/set_banco', {user_id = id_dono, bank = preco_db+tonumber(bank)})
                    end
                ...
                vRPclient._notify(player, "Você já possui um veiculo desse modelo!")
            end
        else
