...
vRP._prepare("NL/get_estoque_mercado",
             "SELECT estoque FROM vrp_mercado WHERE nome = @nome")
vRP._prepare("NL/set_estoque_mercado",
             "UPDATE vrp_mercado SET estoque = @estoque WHERE nome = @nome")
vRP._prepare("NL/get_cofre_mercado",
             "SELECT cofre, cofreLimite FROM vrp_mercado WHERE nome = @nome")
vRP._prepare("NL/set_cofre_mercado",
             "UPDATE vrp_mercado SET cofre = @cofre WHERE nome = @nome")
vRP._prepare("NL/comprar_mercado",
             "UPDATE vrp_mercado SET id_dono = @id_dono, a_venda = @a_venda WHERE nome = @nome")
vRP._prepare("NL/selecionar_mercado_notvenda",
             "SELECT * FROM vrp_mercado WHERE a_venda = 0 AND id_dono <> 0")
vRP._prepare("NL/selecionar_mercado", "SELECT * FROM vrp_mercado")
vRP._prepare("NL/selecionar_mercado_nome",
             "SELECT * FROM vrp_mercado WHERE nome = @nome")

Citizen.CreateThread(function() CriarProdutos() end)
...
function setEstoque(nome, estoque)
    vRP.execute("NL/set_estoque_mercado",
                {nome = nome, estoque = json.encode(estoque)})
end
...
function getMercado(nome)
    ...
    for k, v in pairs(rows) do return v end
end
...
AddEventHandler('Mercado:EnviarProdutos', function(nome)
    ...
    for k, v in pairs(vRP.getInventory(user_id)) do
        inventario[k] = {['amount'] = v.amount, ['name'] = vRP.getItemName(k)}
    end
    ...
    TriggerClientEvent('Mercado:ReceberAnuncio', source, loja, inventario,
                       dinheiro, weight, max_weight,
...
AddEventHandler('Mercado:Comprar', function(nome, loja)
    ...
    for k, v in pairs(produtos) do
        ...
        if estoque[v].quantidade > 0 then
            if ((vRP.getInventoryWeight(user_id) + vRP.getItemWeight(v)) <=
                ...
                if vRP.tryPayment(user_id, tonumber(estoque[v].preco)) then
                    ...
                    if (cofreAtual < limite) then
                        local valorTotal = cofreAtual +
                                               tonumber(estoque[v].preco)
                        setCofre(loja, valorTotal, source)
                    end
                    setEstoque(loja, estoque)
                else
                    vRPclient._notify(source, "[" .. loja ..
...
AddEventHandler('Mercado:Anunciar',
                function(nome, produto, preco, estoque, foto)
    local source = source
    local user_id = vRP.getUserId(source)
    ...
    if amount >= estoque then
        ...
        if pegarEstoque[produto] then
            ...
            pegarEstoque[produto] = {
                nome = vRP.getItemName(produto),
                quantidade = estoque,
                preco = preco
            }
            setEstoque(nome, pegarEstoque)
    ...
    if info.a_venda then
        ...
        if vRP.tryPayment(user_id, tonumber(preco)) then
            ...
            vRPclient._notify(source,
                              "Parabéns! Você comprou o comércio: " .. nome)
        else
