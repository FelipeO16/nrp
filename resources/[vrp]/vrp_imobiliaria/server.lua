...
vRP._prepare("sRP/inserir_casa",
             "INSERT INTO vrp_imobiliaria(nome, preco, img, categoria, a_venda) VALUES(@nome, @preco, @img, @categoria, @a_venda)")
vRP._prepare("sRP/set_venda",
             "UPDATE vrp_imobiliaria SET a_venda = @a_venda WHERE nome = @nome")
vRP._prepare("sRP/selecionar_casa", "SELECT * FROM vrp_imobiliaria")
vRP._prepare("sRP/obter_casa",
             "SELECT * FROM vrp_imobiliaria WHERE nome = @nome")
...
function quantidadeCasa()
    local rows = vRP.query("sRP/selecionar_casa")
    local vendas = vRP.table_size(vRP.table_filter(rows, function(v, k, t)
        return v.a_venda
    end))
    return vendas
end
...
AddEventHandler('Imobiliaria:enviarCasasVenda', function()
    ...
    TriggerClientEvent('Imobiliaria:receberCasasVenda', source, casas,
                       quantidade)
end)
...
AddEventHandler('Imobiliaria:comprarCasa', function(info)
    ...
    if vRP.tryFullPayment(user_id, preco) then
        ...
        vRPclient._notifyPicture(source, "CHAR_ANTONIA", 9, "Imobiliária",
                                 "Atendente Debora",
                                 "Parabéns pela conquista, você comprou sua casa!")
        TriggerEvent('Homes:comprarCasa', user_id, nome)
    else
        vRPclient._notifyPicture(source, "CHAR_ANTONIA", 1, "Imobiliária",
                                 "Atendente Debora",
...
AddEventHandler('Imobiliaria:visitarCasas', function(info)
    ...
    if user_id then
        if not em_visita[id] then
            ...
            em_visita[id] = {['visita'] = true}

            vRPclient._teleport(source, x, y, z)
            vRPclient._notifyPicture(source, "CHAR_ANTONIA", 1, "Imobiliária",
                                     "Atendente Debora",
                                     "Você tem apenas ~g~1 minuto~w~ de visista")

            SetTimeout(60000, function()
                vRPclient._notifyPicture(source, "CHAR_ANTONIA", 1,
                                         "Imobiliária", "Atendente Debora",
                                         "Seu tempo de visita acabou!")
        ...
        elseif em_visita[id].visita then
            vRPclient._notifyPicture(source, "CHAR_ANTONIA", 1, "Imobiliária",
                                     "Atendente Debora",
                                     "Já possui uma pessoa visitando essa casa, aguarde!")
        end
    end
