...
function vRPc.spawnVeiculo(vehicle)
    ...
    local motor, lataria, gasolina = vehicle.motor, vehicle.lataria,
                                     vehicle.gasolina

    local mhash = GetHashKey(veh)
    RequestModel(mhash)
    while not HasModelLoaded(mhash) do Citizen.Wait(0) end
    ...
    while not NetworkHasControlOfEntity(nveh) do Citizen.Wait(100) end
...
AddEventHandler('Concessionaria:receberVeiculos',
                function(veiculos, garagem, identidade)
    ...
    if CNserver.PermVip() then SendNUIMessage({vip = true}) end
end)
...
RegisterNUICallback('anunciar', function(data, cb)
    ...
    local img = data.img
    TriggerServerEvent('Concessionaria:anunciarVeiculo', veh, nome, tipo, qtd,
                       desc, img)
end)
...
Citizen.CreateThread(function()
    ...
    while true do
        Citizen.Wait(0)
        DrawMarker(27, -29.842, -1104.661, 26.500 - 1.0001, 0, 0, 0, 0, 0, 0,
                   1.0001, 1.0001, 1.0001, 0, 0, 0, 0.8, 0, 0, 0, 0, 0, 0, 0)
        if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), -29.842,
                                    -1104.661, 26.422, true) < 1 then
