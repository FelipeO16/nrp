...
vRP._prepare("sRP/inserir_veh","INSERT INTO vrp_concessionaria(modelo,nome,tipo,preco,quantidade,descricao,img) VALUES(@modelo,@nome,@tipo,@preco,@quantidade,@descricao,@img)")
vRP._prepare("sRP/set_quantidade","UPDATE vrp_concessionaria SET quantidade = @quantidade WHERE modelo = @modelo")
vRP._prepare("sRP/selecionar_veh","SELECT * FROM vrp_concessionaria")
vRP._prepare("sRP/get_veh_by_nome","SELECT * FROM vrp_concessionaria WHERE modelo = @nome")
vRP._prepare("sRP/get_quantidade","SELECT quantidade FROM vrp_concessionaria WHERE modelo = @modelo")
...
function vRPc.getCarImage(model)
	return cars[tostring(model)].imagemcarro
end

function vRPc.getCarInfo(model)
	if not cars[tostring(model)] then print("Não foi possível obter o carro: "..model) end
...
AddEventHandler('Concessionaria:enviarVeiculos', function()
    ...
    local gah = vRP.query("vRP/get_vehicles_by_states", {user_id = user_id, state = false})
    ...
    TriggerClientEvent('Concessionaria:receberVeiculos', source, veiculos, garagem, identity.name.." "..identity.firstname)
end)

function criarPlaca()
    local generatePlaca
    generatePlaca = "P"..math.random(10000, 99999)
	local veiculos = vRP.query('vRP/selecionar_veh_placa', {placa = generatePlaca})
    if #veiculos == 0 then
...
AddEventHandler('Concessionaria:comprarCarro', function(veh)
	...
	if count(rows) < maxcars then
		...
		local imagem = veiculo.imagemcarro
		local peso = veiculo.pesocarro
		local rows = vRP.query("sRP/get_quantidade", {modelo = veh})
		local quantidade
		if #rows > 0 then
			quantidade = rows[1].quantidade
		end
		local get_veh = vRP.query("vRP/get_vehicle", {user_id = user_id, vehicle = veh})
		if count(get_veh) <= 0 then
			if quantidade > 0 then
				local generatePlaca = criarPlaca()
				if vRP.tryFullPayment(user_id, valor) then
					TriggerEvent("whook:sendhook",10,tostring(tostring(GetPlayerName(source))) .. " [" .. user_id .. "]".." Pagou:  "..tostring(valor).." em um "..tostring(nome))
					quantidade = quantidade - 1		
					-- Seta a quantidade atualizada na concessionaria
					vRP.execute("sRP/set_quantidade", {modelo = veh, quantidade = quantidade})
					-- Teste
					...
					if vRP.getSData("concessionaria:dono") ~= nil then
						vRP.setSData("concessionaria:banco", (tonumber(vRP.getSData("concessionaria:banco")) or 0)+valor*0.15)
					end
					TriggerClientEvent("Notify",source, "sucesso","Parabéns, você comprou um <b>"..nome.."</b>,Modelo: "..modelo..", Placa: "..generatePlaca)
				else
		...
		TriggerClientEvent("Notify",source, "negado", "Sua garagem está lotada!")
	end
end)

RegisterServerEvent('Concessionaria:anunciarVeiculo')
AddEventHandler('Concessionaria:anunciarVeiculo', function(veh,nome,tipo,qtd,desc,img)
    local source = source
    local user_id = vRP.getUserId(source)
	...
	if vRP.hasPermission(user_id,"adm.perm") then
		...
		if uid and car then
			...
			if info then
				...
				-- Adiciona o veiculo no banco de dados
				vRP.execute("vRP/add_vehicle", createVehicle)
				TriggerClientEvent("Notify",source, "sucesso", "Você deu o carro "..car.." para o ID "..uid)
			else
				TriggerClientEvent("Notify",source, "negado", "Nome do carro invalido")
			end
		else
			TriggerClientEvent("Notify",source, "negado", "Utilize /givecar id carro")
		end
	end
end)
