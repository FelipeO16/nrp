...
vRP._prepare("NL/inserir_apreensao","INSERT INTO vrp_cet(dono, dono_nome, modelo, placa, valor, slot) VALUES(@dono, @dono_nome, @modelo, @placa, @valor, @slot)")
...
function spawnarCarro(user_id, veiculo)
    if user_id then
        ...
        local rows = vRP.query("vRP/selecionar_veh_placa", {placa = veiculo.placa})
        if rows then
	...
	if not vRPclient.isHandcuffed(player) then
		...
		if type(k) ~= 'nil' then
			...
			if vRP.hasGroup(user_id, "CET") then
				local placa,modelo,veh = vRPclient.getVehicleAtRaycast(player, 1)
				if placa then
					local rows = vRP.query("vRP/selecionar_veh_placa", {placa = placa})
					if #rows > 0 then
						print("Aqui", rows[1].user_id, rows[1].placa, rows[1].tipo)
						local id = rows[1].user_id
						if id then
							print("ID: "..id)
							local a_venda = vRP.query("NL/get_venda_by_placa", {placa = placa})
							if #a_venda <= 0 then
								...
								if not apreendido then
									local valor = vRP.prompt(player, "Digite a categoria da multa","")
									local final = tostring(valor)
									if categoria[final] then

										local veiculo = {
											['dono'] = id,
											['dono_nome'] = identidade.name.." "..identidade.firstname,
										...
										vRP.execute("NL/inserir_apreensao", veiculo)
										...
										local carros = json.decode(vRP.getSData("apreendido:u"..id))
										if not carros then carros = {} end
										carros[modelo] = true
										vRP.setSData("apreendido:u"..id, json.encode(carros))  
										print("Id: "..id, "Player: "..player, "Veh: "..veh)
										-- Deleta o veiculo
										TriggerClientEvent("deletarveiculo", player, veh)
										-- Spawna o carro
										spawnarCarro(user_id, veiculo)
										-- Notify
										vRPclient._notify(player, "[Pátio] Veiculo apreendido com sucesso!")
									else
										vRPclient._notify(player, "[Pátio] Essa categoria não existe!")
									end
								else
									vRPclient._notify(player, "[Pátio] Esse veiculo já está apreendido")
								end
							else
								vRPclient._notify(player, "[Pátio] Não é possivel apreender esse veiculo!")
							end
						end
					end
				else
					vRPclient._notify(player, "Não existe nenhum veiculo proximo")
				end
			else
		...
		if type(retirar[player]) ~= 'nil' then
			...
			if user_id == id_dono then
				if vRP.tryPayment(user_id, valor) then
					...
					vRPclient._notify(player, "[Pátio] Você pagou: $"..valor.." e seu veiculo foi liberado!")
					...
					vRPclient._notify(player, "[Pátio] Você não possui dinheiro suficiente!")
				end
			else
				vRPclient._notify(player, "[Pátio] Este veiculo não pertence a você!")
			end
		end
	end
end)
