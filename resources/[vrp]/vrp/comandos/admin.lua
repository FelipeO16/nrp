...
RegisterCommand('admin',function(player, choice)
  ...
  if user_id then
    ...
    for k,v in pairs(players) do
      async(function()
        local ok = vRP.request(v,"Chamada [ID: "..user_id.."] atender: "..htmlEntities.encode(desc), 60)
        if ok then 
...
RegisterCommand('removegroup',function(source,args,rawCommand)
	...
	if vRP.hasPermission(user_id,"adm.perm") then
		if args[1] and args[2] then
			...
			vRPclient._notify(source, "Emprego: "..args[2].." foi removido do ID: "..parseInt(args[1]))
		end
	end
end)
	...
	if vRP.hasPermission(user_id,"adm.perm") then
		if args[1] then
     		vRP.setWhitelisted(parseInt(args[1]),true)
      		vRPclient._notify(source, "Você deu whitelist para o ID: "..parseInt(args[1]))
		end
	end
end)
...
RegisterCommand('unwl',function(source,args,rawCommand)
	...
	if vRP.hasPermission(user_id,"adm.perm") then
		if args[1] then
      		...
      		vRPclient._notify(source, "Você tirou a whitelist do ID: "..parseInt(args[1]))
		end
	end
end)
...
RegisterCommand('car',function(source,args,rawCommand)
	...
	if vRP.hasPermission(user_id,"adm.perm") then
		if args[1] then
			vRPclient._SpawnCar(source, args[1])
		end
	end
end)
