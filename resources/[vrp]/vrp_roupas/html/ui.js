...
$(document).ready(function() {
    ...
    $("#confirmar").click(function() {
        $.post('http://vrp_roupas/close', JSON.stringify({}))
    })
    ...
    $(".btn2").click(function() {
        let roupa = roupaID
        $.post('http://vrp_roupas/next_custom', JSON.stringify({ type: roupa }));
        if (!change[roupa]) {
            change[roupa] = 1;
        } else {
            change[roupa] += 1;
    ...
    $(".btn1").click(function() {
        let roupa = roupaID
        $.post('http://vrp_roupas/anterior_custom', JSON.stringify({ type: roupaID }));
        if (!change[roupa]) {
            change[roupa] = -1;
        } else {
            change[roupa] -= 1;
    ...
    $("#right").click(function() {
        $.post('http://vrp_roupas/rotaterightheading', JSON.stringify({
            value: 10
        }));
    });

    $(".btncor").click(function() {
        $.post('http://vrp_roupas/cor', JSON.stringify({ type: roupaID }));
    })
    ...
    $(".comprar").click(function() {
        ...
        $.post('http://vrp_roupas/comprar', JSON.stringify({ preco: total }))
        update_valor()
    })
